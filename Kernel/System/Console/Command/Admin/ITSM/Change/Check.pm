...
sub Configure {
    ...
    $Self->Description('Check ITSM changes.');
    $Self->AddOption(
        Name        => 'force',
        Description => "Start command forced even though it is already running.",
        Required    => 0,
        HasValue    => 0,
    );
...
sub PreRun {
    ...
    # create PID lock
    $Self->{PIDLockName} = 'ITSMChangesCheck';
    my $PIDLock = $PIDObject->PIDCreate( Name => $Self->{PIDLockName} ) // 0;

    if ( !$Self->GetOption('force') && !$PIDLock ) {
        die "NOTICE: Command is already running (Use '--force' if you want to start it forced)!\n";
    }
    elsif ( $Self->GetOption('force') && !$PIDLock ) {
        $Self->Print("\n<yellow>NOTICE: Command is already running but it is starting again!</yellow>\n\n");
    }
...
sub PostRun {
    ...
    # delete pid lock
    $Kernel::OM->Get('Kernel::System::PID')->PIDDelete( Name => $Self->{PIDLockName} );
